...
**Мини-апп (6 вкладок):**
- **Главная** — safe-to-spend; четыре цифры месяца (потрачено/доход/отложено/капитал) со стрелками Δ к прошлому месяцу; «N трат ждут разбора»; прогноз месяца; график трат по месяцам; категории с Δ; последние операции + «Все операции».
- **Операции** — полный список всех транзакций с фильтрами (период/тип/категория/поиск по продавцу), группировкой по дням с дневным итогом, итогом по выборке (расходы/доходы) и пагинацией; тап → правка категорий.
...
- **Капитал** — net worth (мультивалюта по курсу ЦБ) с Δ за период; линия динамики по снимкам; разбивка прироста (сбережения / курс $ / прочее); счета с правкой баланса, долги, запасной доход.
...
| GET/POST `/api/recurring` · DELETE · POST `/api/recurring/dismiss` | регулярные расходы + скрытие кандидатов |
