...
- **Капитал** — net worth (мультивалюта по курсу ЦБ) с Δ; **компактный график курса USD/RUB за год** (текущий курс ЦБ, Δ% за год, мин/макс, hover crosshair); линия динамики капитала; **карточка «Прирост»** с переключателем периодов (День/Неделя/Месяц/Год) и компактной шапкой (число под заголовком, чипы и дата справа); раскладка delta = Сбережения + Курс $ + Прочее, согласованная для выбранного периода; **🔥 Финансовая независимость (FIRE) под РФ в одном блоке с прогнозом**: 3 одинаковых серых плитки сценариев SWR (Осторожный 2%/50× · Базовый РФ 3%/33× · Оптимистичный 4%/25×) с числом лет до FI (28px) и правильным склонением («38 лет», «44 года», «21 год»), цель и SWR/множитель мелким серым, бейдж прогресса в углу; компактная строка «Расходы X/год · Откладываю Y/мес · Реал. Z%»; Runway и Coast FI рядом; **график прогноза капитала** горизонтальной чертой ниже — горизонт автоматически `max(20, лет_до_пенсии + 5)` (для 38-летнего мужчины = до пенсии 2058 + 5 лет = 32 года), пунктиры FI-целей и точки пересечения; **распределение по валюте** + дрейф от целевой аллокации (например 70/30 RUB/USD); **подушка безопасности** (мес расходов); **цель капитала** + прогноз достижения + «отложено в этом месяце»; счета с **бренд-цветами банков** (Тинькофф/Сбер/Райф/Альфа/ВТБ/Газпром/Озон/Яндекс/Почта/МТС), CRUD, баланс инлайн с **поддержкой копеек** (`step=0.01`); **долги** (несколько контрагентов, частичные погашения, прогресс «отдано/осталось», остаток в капитале); **вклады 2.0** — теперь реально входят в net_worth, capitalization (с/без) считается **по-настоящему** (сложный процент vs линейный), а **реальные пополнения через DepositTopup** заменяют плановый monthly_topup: кнопка «+ Пополнить» в карточке вклада, sheet с суммой/датой и списком последних пополнений, строка «В этом месяце доложил Xк» по факту.
...
- **🏦 Счета и активы** — балансы карт/наличных/крипты/вкладов; тап → форма счёта.
...
`accounts` (счета: card/cash/deposit/crypto/external, валюта, владелец me/wife/m1/…, баланс) ·
...
`goals` · `budgets` (лимиты по категориям) · `debts` (paid для частичного погашения) · `deposits` · `deposit_topups` (история фактических пополнений вкладов: deposit_id, date, amount) · `net_worth_snapshots` (зафиксирован в 00:01, неизменен в течение дня) · `fx_rates` · `external_reports` (для сверки жены) · `settings` (key-value: `alert_big`, `networth_target`, FIRE-параметры, `emergency_target_months`, `family_members`, и др.).
...
| GET | `/api/fire` | FIRE-метрики: 3 сценария + Years to FI + Coast FI + Runway + дрейф аллокации + rolling сбережения |
| GET | `/api/forecast?years=N` | прогноз net worth в «сегодняшних рублях» с пересечениями FI-целей; если `years` не задан → горизонт = `max(20, years_to_retire + 5)` |
| GET/POST `/api/deposits` · POST `/api/deposits/{id}` · DELETE | вклады (CRUD: bank, principal, rate, monthly_topup-план, term, capitalization on/off); расчёт = `_grow_compound`(с капит.) или `_grow_simple`(без) от реальных DepositTopup или fallback monthly_topup |
| POST `/api/deposits/{id}/topup` · DELETE `/api/deposits/topup/{id}` | реальные пополнения вклада (date + amount + опц. note) — заменяют плановый monthly_topup в расчёте |
