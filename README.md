...
**Настройки** (шестерёнка на Главной): тема (Авто/Светлая/Тёмная, синхронизация между устройствами через TG CloudStorage); порог «крупной траты» (`alert_big`); **управление категориями** (CRUD: имя, родитель, цвет из палитры 14, иконка из набора 32; пустые удаляются насовсем, с операциями — архивируются); **поиск по чекам ФНС** (по позициям и магазину — «найди где покупал хлеб»); справочно — расписание дайджестов и о приложении.

**UX-плюшки:** глобальный **FAB «+»** на всех вкладках — операция добавляется в два тапа; **skeleton-loaders** вместо «Загрузка…» — воспринимаемая скорость +30%; плавная анимация смены вкладок (fade); **sticky-header** при скролле длинных страниц; **TG BackButton** автосинхронизируется со шторками; haptic-feedback в ключевых местах; **AI-ассистент** на Gemini по своим данным с историей вопросов (localStorage); **smart-compose** автоподсказки магазинов в форме новой операции.
