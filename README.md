...
- **Капитал** — net worth (мультивалюта по курсу ЦБ) с Δ; **компактный график курса USD/RUB за год** (текущий курс ЦБ, Δ% за год, мин/макс, hover crosshair); линия динамики капитала; разбивка прироста (сбережения / курс $ / прочее); **🔥 Финансовая независимость (FIRE) под РФ**: 3 сценария SWR (Осторожный 2%/50× · Базовый РФ 3%/33× · Оптимистичный 4%/25×) с FI number, прогрессом и Years to FI; Runway без дохода; **Coast FI** до пенсии (авто-расчёт по дате рождения + пол с учётом реформы 2018); **прогноз капитала на 5/10/20 лет** в «сегодняшних рублях» с пунктирами FI-целей и точками пересечения; **распределение по валюте** + дрейф от целевой аллокации (например 70/30 RUB/USD); **подушка безопасности** (мес расходов); **цель капитала** + прогноз достижения + «отложено в этом месяце»; счета с **бренд-цветами банков** (Тинькофф/Сбер/Райф/Альфа/ВТБ/Газпром/Озон/Яндекс/Почта/МТС — узнавание с первого взгляда), CRUD, баланс инлайн; **долги** (несколько контрагентов, частичные погашения, прогресс «отдано/осталось», остаток в капитале); **вклады с доходностью** (ставка, прогноз, начисленные %).
...
**Настройки** (шестерёнка на Главной): тема (Авто/Светлая/Тёмная, синхронизация между устройствами через TG CloudStorage); порог «крупной траты» (`alert_big`); **семья** (переименование «Я»/«Жена» в произвольные имена + добавление новых членов семьи — везде где владелец счёта/дохода/вклада); **управление категориями** (CRUD: имя, родитель, цвет из палитры 14, иконка из набора 32; пустые удаляются насовсем, с операциями — архивируются); **поиск по чекам ФНС** (по позициям и магазину — «найди где покупал хлеб»); справочно — расписание дайджестов и о приложении.
...
| GET | `/api/forecast?years=20` | прогноз net worth в «сегодняшних рублях» с пересечениями FI-целей |
