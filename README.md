...
**UX-плюшки:** глобальный **FAB «+»** на всех вкладках (автоскрывается при скролле вниз, возвращается на скролл вверх — не перекрывает контент); **hover-тултипы для десктоп-Telegram** на всех графиках (heatmap, курс USD/RUB, линия капитала, cashflow, bar-чарт, hbar-сегменты, топ-продавцы) с vertical crosshair на линейных графиках + **click-to-drill** (тап по дню/месяцу → операции с фильтром); **skeleton-loaders** вместо «Загрузка…»; плавная анимация смены вкладок (fade); **TG BackButton** автосинхронизируется со шторками; haptic-feedback в ключевых местах; **AI-ассистент** на Gemini по своим данным с историей вопросов (localStorage); **smart-compose** автоподсказки магазинов в форме новой операции.
...
backend/
  app/
    ...
    services/
      ...
      deposits.py      # вклады: доходность, прогноз, начисленные %; deposits_total_value в RUB
